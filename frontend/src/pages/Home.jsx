...
import { getLeaderboard, getCommunityStats } from '../utils/api';
...
export default function Home() {
  ...
  const pointsToday = rewards?.transactions
    ?.filter(t => t.type === 'earn' && new Date(t.timestamp).toDateString() === new Date().toDateString())
  ...
  useEffect(() => {
    getScanHistory().then((h) => {
      ...
      // total_disposed from rewards = completed take-backs; fallback to scan estimate
      const disposed = rewards?.total_disposed ?? Math.floor(h.length * 0.8);
      // Add take-back items count to scans for total medicines logged
      const takeBackCount = rewards?.transactions
        ?.filter(t => t.reason?.includes('takeback') || t.reason?.includes('disposal'))
        ?.length || 0;
      setStats({
        total: h.length + takeBackCount,
        antibiotics: h.filter((s) => s.is_antibiotic).length,
        disposed,
      });
    });
    getLeaderboard().then(setLeaderboard).catch(() => {});
    getCommunityStats().then(setCommunityStats).catch(() => {});
  }, [rewards?.total_disposed, rewards?.transactions]);
  ...
  return (
    <div className="page">
      ...
      {/* Points Earned Today Banner */}
      {pointsToday > 0 && (
        <div className="animate-fade-in flex items-center justify-between rounded-2xl px-5 py-4 mt-6 gradient-primary shadow-accent text-white">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center backdrop-blur-sm">
              <Icons.Star className="w-5 h-5 text-yellow-300" />
            </div>
            <div>
              <p className="font-bold text-[15px]">{t('points_earned_today')}</p>
              <p className="text-[12px] text-white/80">Keep up the great work!</p>
            </div>
          </div>
          <div className="text-2xl font-black text-yellow-300">+{pointsToday}</div>
        </div>
      )}
      ...
      <div className="animate-fade-in-up stagger-1 mt-7">
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: 'Medicines Logged', value: stats.total, Icon: Icons.Scan, color: '#0d9488' },
      ...
      <div className="animate-fade-in-up stagger-2">
        ...
        <div className="grid grid-cols-4 gap-2">
          {[
            ...
            { Icon: Icons.Recycle, label: t('takeback'), action: () => navigate('/takeback'), color: '#22c55e' },
