...
import { analyzeMedication, getGeminiStatus } from '../utils/api';
...
function ExpiryMeter({ score, label }) {
  const color = score >= 9 ? '#7f1d1d' : score >= 7 ? '#ef4444' : score >= 4 ? '#d97706' : '#22c55e';
  return (
    <div className="flex items-center gap-3 mt-2">
      <div className="flex-1 h-2.5 rounded-full overflow-hidden" style={{ background: 'var(--bg-tertiary)' }}>
        <div className="h-full rounded-full transition-all duration-700" style={{ width: `${score * 10}%`, background: color }} />
      </div>
      <span className="text-xs font-bold shrink-0" style={{ color }}>Risk: {score}/10 — {label}</span>
    </div>
  );
}

function ErsCard({ ersData }) {
  if (!ersData) return null;
  const { score, risk_level, risk_color, risk_description, persistence_score,
    bioaccumulation_score, persistence_factor, amr_multiplier, formula_values,
    months_expired, expiry_known } = ersData;

  const tierLabels = ['—', 'Low Impact', 'Moderate Risk', 'High Risk', 'Critical Risk'];
  const tierBgs = ['', 'rgba(34,197,94,0.1)', 'rgba(245,158,11,0.1)', 'rgba(239,68,68,0.1)', 'rgba(127,29,29,0.15)'];

  return (
    <div className="card p-5" style={{ borderLeft: `3px solid ${risk_color}` }}>
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <span style={{ fontSize: '16px' }}>🌿</span>
          <p className="text-xs uppercase tracking-wider font-semibold" style={{ color: 'var(--text-tertiary)' }}>ERS — Environmental Risk Score</p>
        </div>
        <span style={{ fontSize: '11px', fontWeight: '800', padding: '3px 10px', borderRadius: '20px', background: `${risk_color}20`, color: risk_color, whiteSpace: 'nowrap' }}>
          {risk_level}
        </span>
      </div>
      ...
      <div style={{ marginBottom: '14px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
          ...
          <span style={{ fontSize: '14px', fontWeight: '800', color: risk_color }}>{score}/10</span>
        </div>
        <div style={{ height: '8px', borderRadius: '8px', background: 'var(--bg-tertiary)', overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${(score / 10) * 100}%`, background: `linear-gradient(90deg, ${risk_color}80, ${risk_color})`, borderRadius: '8px', transition: 'width 0.8s ease' }} />
        </div>
      </div>

      {/* Formula breakdown — only for expired medicines */}
      {expiry_known && score > 0 && persistence_score !== null && (
        <>
          <div style={{ padding: '10px 12px', borderRadius: '10px', background: 'var(--bg-secondary)', marginBottom: '12px' }}>
            <p style={{ fontSize: '10px', color: 'var(--text-tertiary)', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '6px' }}>Formula: ERS = (P + B) × PF × AMR</p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
              {[
                { label: 'Persistence (P)', value: `${persistence_score}/3`, tip: 'Drug breakdown rate in environment' },
                { label: 'Bioaccumulation (B)', value: `${bioaccumulation_score}/3`, tip: 'XLogP-based fat tissue affinity' },
                { label: 'Time Factor (PF)', value: persistence_factor, tip: `${months_expired}mo expired` },
                { label: 'AMR Multiplier', value: `×${amr_multiplier}`, tip: amr_multiplier > 1 ? 'Antibiotic penalty' : 'Standard drug' },
              ].map(({ label, value, tip }) => (
                <div key={label} style={{ flex: '1 1 calc(50% - 6px)', minWidth: '130px', background: 'var(--bg-card)', borderRadius: '8px', padding: '8px 10px', border: '1px solid var(--border-subtle)' }}>
                  <p style={{ fontSize: '9px', color: 'var(--text-tertiary)', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: '2px' }}>{label}</p>
                  <p style={{ fontSize: '15px', fontWeight: '800', color: risk_color }}>{value}</p>
                  <p style={{ fontSize: '9px', color: 'var(--text-tertiary)', marginTop: '1px' }}>{tip}</p>
                </div>
              ))}
            </div>
            {formula_values && (
              <p style={{ fontSize: '11px', fontWeight: '700', color: 'var(--text-secondary)', marginTop: '8px', fontFamily: 'monospace', textAlign: 'center' }}>
                = {formula_values}
              </p>
            )}
          </div>
      ...
      {/* Risk description */}
      <p style={{ fontSize: '12px', color: 'var(--text-secondary)', lineHeight: '1.5' }}>{risk_description}</p>
    </div>
  );
}
...
export default function Scanner() {
  ...
  useEffect(() => { getGeminiStatus().then(setGeminiStatus); }, []);
          ...
          <div className="absolute top-16 left-0 right-0 text-center">
            ...
            {geminiStatus && (
              <div style={{
                ...
                background: geminiStatus.active ? 'rgba(34,197,94,0.25)' : 'rgba(245,158,11,0.25)',
                border: `1px solid ${geminiStatus.active ? 'rgba(34,197,94,0.5)' : 'rgba(245,158,11,0.5)'}`,
              }}>
                <span style={{ fontSize: '9px', fontWeight: '800', color: geminiStatus.active ? '#4ade80' : '#fbbf24', letterSpacing: '0.05em', textTransform: 'uppercase' }}>
                  {geminiStatus.active ? '✦ Gemini Vision AI Active' : '⚡ Demo Mode'}
                </span>
              </div>
            )}
          </div>
  ...
  return (
    <div className="px-4 pt-6 pb-6">
      ...
      {phase === 'analyzing' && (
        <LoadingTips district={district} title={t('analyzing')} />
      )}
      ...
      {phase === 'result' && result && (
        <div className="space-y-4 animate-fade-in-up">
          ...
          <div className="card p-5" style={{ borderLeft: `3px solid ${result.is_antibiotic ? '#ef4444' : 'var(--accent)'}` }}>
            ...
            <div className="grid grid-cols-2 gap-3 mt-4">
              <div className="card p-3">
                ...
                {geminiStatus?.active && <p className="text-[9px] mt-0.5" style={{ color: '#4ade80' }}>✦ AI Scanned</p>}
              </div>
              <div className="card p-3">
          ...
          {/* Expiry Risk Meter (if expired) */}
          {result.expiry_status === 'expired' && (
            <div className="card p-5" style={{ borderLeft: '3px solid #ef4444' }}>
              <div className="flex items-center gap-2 mb-3">
                <Icons.AlertTriangle className="w-4 h-4" style={{ color: '#ef4444' }} />
                <p className="text-xs uppercase tracking-wider font-semibold" style={{ color: 'var(--text-tertiary)' }}>Expiry Risk Meter</p>
              </div>
              <ExpiryMeter score={result.expiry_risk_score} label={result.expiry_risk_label} />
              <p className="text-sm leading-relaxed mt-3" style={{ color: 'var(--text-secondary)' }}>
                {result.months_expired > 0 ? `Expired ${result.months_expired} months ago.` : 'Expired recently.'} High expiry risk can lead to toxic byproducts or ineffective treatment.
              </p>
            </div>
          )}

          {/* ERS Card — after expiry section, before eco-hazard */}
          <ErsCard ersData={result.ers_data} />
          ...
          <div className="space-y-3 pt-2">
            <button onClick={() => navigate('/takeback', { state: { drugName: result.drug_name, expiryDate: result.expiry_date, isAntibiotic: result.is_antibiotic } })}
                    className="w-full py-4 rounded-2xl bg-green-500 text-white font-bold text-sm flex items-center justify-center gap-2 active:scale-[0.98] transition-transform shadow-lg shadow-green-500/30">
              <Icons.Recycle className="w-4 h-4" />
              {t('request_takeback')}
            </button>
